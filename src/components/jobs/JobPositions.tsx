...
import { getExperienceLabel } from "@/helpers/experince";
...
export const JobPositions = ({ positions, onPositionSelect, appliedPositions }: { positions: Position[], appliedPositions: string[], onPositionSelect: (positionId: string, checked: boolean) => void }) => {
  ...
  const handlePositionSelect = (positionId: string, e: any) => {
    onPositionSelect(positionId, e.target.checked);
  };
              ...
              <tr key={position._id}>
                <td className={styles.title}>
                  ...
                  <div className="d-inline-block">
                    {position.title}
                    <h6>({getExperienceLabel(position.experience)})</h6>
                  </div>
                </td>
