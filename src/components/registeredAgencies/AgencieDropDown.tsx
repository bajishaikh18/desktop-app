        ...
        <div className={styles.jobListingContainer}>
            <div className={styles.jobList}>
                ...
                {selectedState && (
                    <div className={styles.dropdownMenu}> 
                        {stateOptions
                            .filter((state) => state.value === selectedState)
                            .map((state) => (
                                <div key={state.value}>
                                    <div 
                                        key={state.value} 
                                        className={`${styles.stateItem} ${styles.cityItem}`} 
                                        onClick={() => selectCity(state.value)} 
                                    >
                                        {state.label}
                                        <img
                                            src="/downarrow.png"
                                    ...
                                    {state.cities.map((city) => (
                                        ...
                                        >
                                            {city.label}
                                            {selectedCity === city.value && <span> (Selected)</span>} 
                                        </div>
                                    ))}
                                </div>
                            ))}
